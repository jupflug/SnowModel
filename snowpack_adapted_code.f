...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  multilayer_snowpack,seaice_run,seaice_conc,k_snow,
     &  fc_param,t_avg,Fcapil,mw_exp,ml_ret,retent,
     &  liqfrac,icefrac,grain_sz)
      ...
      real ro_snowmax,tsls_threshold,dz_snow_min,Cp_snow,k_snow,
     &  fc_param,t_avg,th_cond,Fcapil,mw_exp
      ...
      real T_old_z(nz_max)
      real gamma_z(nz_max)
      real layerFlux_z(nz_max)
      real ml_ret_z(nz_max)
      real liqfrac_z(nz_max)
      real icefrac_z(nz_max)
     ...
     &  mLayerVolFracLiqTrial(nx_max,ny_max),retent(nx_max,ny_max)
...
c   not have to do this (I could pass in subsections of the arrays).
          if (multilayer_snowpack.eq.1) then
            do k=1,nz_max
              ...
              layerFlux_z(k) = layerFlux(i,j,k)
              ml_ret_z(k) = ml_ret(i,j,k)
              liqfrac_z(k) = liqfracml(i,j,k)
              icefrac_z(k) = icefracml(i,j,k)
            enddo
          endif
     ...
     &      Cp_snow,seaice_run,liqfrac(i,j),icefrac(i,j),Tsnow(i,j),
     &      multResid(i,j),pore_space(i,j),mLayerThetaResid(i,j),
     &      availCap(i,j),relSaturn(i,j),
     &      mLayerVolFracLiqTrial(i,j),layerFlux_z,k_snow,fc_param,
     &      t_avg,th_cond,Fcapil,mw_exp,Cp_water,ml_ret_z,retent(i,j),
     &      liqfrac_z,icefrac_z,grain_sz)
...
c Re-build the 3-D arrays.  See note above about using f95 to avoid this.
          if (multilayer_snowpack.eq.1) then
            do k=1,nz_max
              ...
              layerFlux(i,j,k) = layerFlux_z(k)
              ml_ret(i,j,k) = ml_ret_z(k)
              liqfracml(i,j,k) = liqfrac_z(k)
              icefracml(i,j,k) = icefrac_z(k)
            enddo
          endif
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  Cp_snow,seaice_run,liqfrac,icefrac,Tsnow,multResid,
     &  pore_space,mLayerThetaResid,availCap,relSaturn,
     &  mLayerVolFracLiqTrial,layerFlux,k_snow,fc_param,t_avg,
     &  th_cond,Fcapil,mw_exp,Cp_water,ml_ret,retent,liqfracml,
     &  icefracml,grain_sz)
     ...
     &  Cp_water
      ...
      real T_old(nz_max)
      real gamma(nz_max)
      real layerFlux(nz_max)
      real ml_ret(nz_max)
      real liqfracml(nz_max)
      real icefracml(nz_max)
      real grain_sz
     ...
     &  swe_depth_old,canopy_int_old,sprec_grnd_ml,seaice_run,Tsnow,
     &  liqfrac,icefrac,multResid,pore_space,mLayerThetaResid,
     &  availCap,relSaturn,mLayerVolFracLiqTrial,k_snow,fc_param,
     &  t_avg,th_cond,Fcapil,mw_exp,retent
...
c Call the multi-layer snowpack model.
     ...
     &    ro_snow_grid,xro_snow,swesublim,A1,A2,layerFlux,k_snow,
     &    fc_param,Fcapil,mw_exp,Cp_water,ml_ret,liqfracml,
     &    icefracml,grain_sz)
...
c Compute the snow density change due to settling.
     ...
     &    snow_depth,ro_water,ro_ice,Tsnow,t_avg,th_cond,Cp)
...
c   the snowpack depth, density, and snow water equivalent.
     ...
     &     sum_swemelt,corr_factor,icorr_factor_index,swesublim,
     &     ro_snowmax,availCap,fc_param,Fcapil,mw_exp,Tf,
     &     Tsnow,retent,grain_sz)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc;cccccc
     ...
     &  sum_sfcsublim,sum_swemelt,corr_factor,potmelt_tmp,availCap,
     &  fc_param,Fcapil,k_snow,mw_exp,Tf,Tsnow

      real icefrac,liqfrac,mLayerTheta,mLayerThetaResid,
     &  mLayerVolFracLiqTrial,multResid,pore_space,
     &  relSaturn,liqflux,sumup,thetaRet,retent,grain_sz
...
c   associated swe depth.
      ...
      mLayerTheta = swe_depth/snow_depth
...
c J.PFLUG 
c inefficient for coding, but to keep single-layer format
c percolation is calculated seperately for each scenario
c in a similar fashion
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c Have rain precipitation
c Have snowpack
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c move melted swe to out of the snowpack and into transferable liq
            swe_depth = swe_depth - swemelt
            sumup = swemelt + rain + canopy_unload
c determine snowpack properties
            CALL UPDATE_STATE(Tsnow,mLayerTheta,liqfrac,icefrac,
     &        fc_param,Tf,ro_water,k_snow,retent,grain_sz)
c transferable water in fractional form
            mLayerVolFracLiqTrial = liqfrac+(sumup/snow_depth)
            if (mLayerVolFracLiqTrial.gt.1.0) then
              mLayerVolFracLiqTrial = 1.0
            endif
c calculate fluxes
            CALL SNOWLIQFLX(dt,mLayerVolFracLiqTrial,liqfrac,
     &        icefrac,liqflux,multResid,pore_space,
     &        mLayerThetaResid,availCap,relSaturn,k_snow,
     &        thetaRet,grain_sz,retent)
c since fluxes depend on time,hydraulic conductivity, saturation (%),
c and the meltwater exponent, results can blow up for high
c saturations/timesteps. Constrain. 
            if (liqflux.gt.sumup) then
              liqflux = sumup
            endif
c manage water fluxes
            swe_depth = swe_depth + sumup - liqflux
            runoff = runoff + liqflux
c will always be true in this case
c            if (snow_depth.gt.0.0) then
...
c We have snow precipitation (on either snow or bare ground).
          ...
          if (snow_depth.gt.0.0) then
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c Have snow precipitation
c have snowpack
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            swe_depth = swe_depth - swemelt
            sumup = swemelt + canopy_unload
            CALL UPDATE_STATE(Tsnow,mLayerTheta,liqfrac,icefrac,
     &        fc_param,Tf,ro_water,k_snow,retent,grain_sz)
            mLayerVolFracLiqTrial = liqfrac+(sumup/snow_depth)
            if (mLayerVolFracLiqTrial.gt.1.0) then
...
c   there is snow falling from the canopy to the snowpack.
        ...
        if (snow_depth.gt.0.0) then
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c Have no precipitation except what is falling from canopy
c have a snowpack
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
          swe_depth = swe_depth - swemelt
          sumup = swemelt + canopy_unload
...
c      snow_depth = swe_depth * ro_water / ro_snow_grid
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  swe_depth_star,ro_ice,ro_water,swe_depth,Tsfc_old,
     &  t_avg,th_cond,Cp

c J.PFLUG
c determine the avg. snow temperature via an exponential decay function
      CALL SNOWTEMP(dt,Cp,snow_depth,Tsfc,t_avg,th_cond,
     &  ro_snow_grid,Tsnow,Tf)
c END J.PFLUG
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  Qe,sfc_sublim_flag,sum_sfcsublim,soft_snow_d,ro_snow,
     &  sum_sprec,sprec_grnd_ml,sum_prec,prec,sum_runoff,
     &  ro_snow_grid,xro_snow,swesublim,A1,A2,layerFlux,k_snow,fc_param,
     &  Fcapil,mw_exp,Cp_water,ml_ret,liqfrac,icefrac,grain_sz)
      ...
      real T_old(nz_max)
      real gamma(nz_max)
      real mLayerVolFracLiq(nz_max)
      real mLayerVolFracIce(nz_max)
      real layerFLux(nz_max)
      real layertheta(nz_max)
      real liqflux(nz_max)
      real mLayerVolFracLiqTrialML(nz_max)
      real ml_ret(nz_max)
      real liqfrac(nz_max)
      real icefrac(nz_max)
      real grain_sz
     ...
     &  ro_snow_grid,xro_snow,swesublim,A1,A2,extra,availCap,
     &  delta_T,extra_delta_T,
     &  mLayerThetaResid,multresid,pore_space,
     &  relSaturn,Tk,k_snow,fc_param,Fcapil,mw_exp,Cp_water
...
c Distribute surface melt and rain precipitation through the snowpack.
     ...
     &  sfc_sublim_flag,sum_sfcsublim,swesublim,extra,fc_param,
     &  Fcapil,mw_exp,Cp_water,ml_ret,liqfrac,icefrac,grain_sz)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  xLsublim,potsublim,swesublim,sum_sfcsublim,Cp_water
      ...
      real fc_param,k_snow,fcapil,mw_exp,extrarun,
     &  mLayerThetaResid,multResid,pore_space,relSaturn,
     &  availCap,delta_T2,thetaRet
...
c densities and temperatures
     ...
     &      liqfrac(j),icefrac(j),fc_param,Tf,ro_water,k_snow,
     &      ml_ret(j),grain_sz)
...
c determine the saturated flux
          CALL SNOWLIQFLX(dt,extra,liqfrac(j),icefrac(j),
     &      liqflux,multResid,pore_space,mLayerThetaResid,
     &      availCap,relSaturn,k_snow,thetaRet,grain_sz,
     &      ml_ret(j))

c for unstable cases where the snowpack completely melts in
c the timestep
          if (liqflux.gt.sumup) then
            liqflux = sumup
          endif
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  liqfrac,icefrac,liqflux,multResid,
     &  pore_space,mLayerThetaResid,availCap,
     &  relSaturn,k_snow,thetaRet,grain_sz,retent)
...
c public variables
      ...
      real Fcapil,k_snow,mw_exp,pore_space,multResid,residThrs,
     &  residScal,availCap,relSaturn,iden_ice,mLayerThetaResid,
     &  thetaRet,grain_sz,m,n,retent
...
c compute the residual volumetric liquid water content
c        mLayerThetaResid = retent * pore_space
        mLayerThetaResid = retent
...
c note that this uses the hydraulic conductivity which
c is set in the parameter file
           liqflux = dt*k_snow*relSaturn**3.0
c           print *,dt,k_snow,relSaturn,liqflux
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  mLayerVolFracIce,fc_param,Tf,ro_water,k_snow,
     &  retent,grain_sz)
     ...
     &  ro_water,k_snow,grain_sz,retent
...
c fraction of liquid water
     ...
     &  min(Tk,Tf)))**2.0)
c      fracliquid = 1.0/(1.0 + (fc_param*(Tf -
c     &  min(Tk,273.155)))**2)
...
c calculate the hydraulic conductivity
      k_snow = (0.00077)*(grain_sz**2)*(9.8/1.787)*
     &  exp(-0.0078*mLayerTheta*1000)
      return
      end

cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      SUBROUTINE SNOWTEMP(dt,Cp,snow_depth,Tsfc,t_avg,th_cond,
     &  ro_snow_grid,Tsnow,Tf)

      implicit none

      real dt,Cp,snow_depth,Tsfc,t_avg,th_cond,Tsnow,ro_snow_grid

      real tamp,depth_0,temp_col,temp_sum,Tf
      integer i

      if (snow_depth.gt.0.0) then

c amplitude of difference between the surface temperature
c and the average snowpack temperature
        tamp = Tsfc - t_avg
c the depth at which surface temperature no longer singnificantly
c influences the snowpack temperature
        depth_0 = sqrt((2*th_cond)/(Cp*ro_snow_grid/dt))*100

c instantiate the summed temperature
        temp_sum = 0.0

c iterating on every centimeter of the snowpack
        do i = 1,floor(snow_depth*100)

c calculate the temperature at the depth of i (cm)
          temp_col = t_avg + (tamp*exp(-1*i/depth_0))

c restrict snowpack temperature to freezing
          if (temp_col.gt.Tf) then
            temp_col = Tf
          endif
          temp_sum = temp_sum + temp_col

        enddo

c determine the snowpack temperature from the average
        Tsnow = temp_sum/floor(snow_depth*100)
