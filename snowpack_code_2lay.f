...
c      CALL MELT_SNOW_ML(JJ,swe_lyr,ro_water,ro_layer,Qm,dt,
c     &  dy_snow,ro_snowmax,rain,xLf,Cp_snow,Tf,T_old,melt_flag,
c     &  runoff,canopy_unload,swe_depth,snow_depth,vegtype,
c     &  glacier_melt,sum_glacmelt,sum_swemelt,soft_snow_d,Qe,
c     &  sfc_sublim_flag,sum_sfcsublim,swesublim,fc_param,
c     &  Cp_water,ml_ret,
c     &  change_layer,dz_snow_min,gamma,max_layers,prec,ro_nsnow,
c     &  ro_snow,ro_snow_grid,snow_d,sprec_grnd_ml,sum_prec,
c     &  sum_runoff,sum_sprec,tsfc,tsls_threshold,tslsnowfall,undef,
c     &  xro_snow)

      CALL MELT_SNOW_ML_2LAY(JJ,swe_lyr,ro_water,ro_layer,Qm,dt,
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      SUBROUTINE MELT_SNOW_ML_2LAY(JJ,swe_lyr,ro_water,ro_layer,Qm,dt,
     &  dy_snow,ro_snowmax,rain,xLf,Cp_snow,Tf,T_old,melt_flag,
     &  runoff,canopy_unload,swe_depth,snow_depth,vegtype,
     &  glacier_melt,sum_glacmelt,sum_swemelt,soft_snow_d,Qe,
     &  sfc_sublim_flag,sum_sfcsublim,swesublim,fc_param,
     ...
     &  ro_layer_old(nz_max),melt_flag_old(nz_max)
      integer JJ_old,b,k
      real sprec_grnd_ml_new,prec_new,runoff_sumup,Tdiff
...
c Initialize the dynamic timestep iteration
c      iteration = 1
c      swe_depth_old = swe_depth
c      snow_depth_old = snow_depth
c      JJ_old = JJ
c      do j=1,JJ
c        swe_lyr_old(j) = swe_lyr(j)
c        dy_snow_old(j) = dy_snow(j)
c        ro_layer_old(j) = ro_layer(j)
c        melt_flag_old(j) = melt_flag(j)
c      enddo
c      sum_glacmelt_old = sum_glacmelt
c      runoff_old = 0.0
c      sum_swemelt_old = sum_swemelt

c if no convergence, return to this point
c8888  dt_new = dt/iteration
c      runoff_sumup = 0.0
...
c if just one layer (<= 20 cm)
      if (JJ.eq.1) then
c all water out of the layer goes to runoff
        runoff = swemelt+rain
c temperature warms due to rain in
        delta_T = (rain*xLf)/(Cp_snow*dy_snow(1))
c adjust temperature
        T_old(1) = T_old(1) + delta_T
c if temperature above Tf
        if (T_old(1).ge.Tf) then
          T_old(1) = Tf
          melt_flag = 1
        else
          melt_flag = 0
...
c temperature diff on top layer from rain in
c        delta_T = (rain*xLf)/(Cp_snow*dy_snow(2))
c        T_old(2) = T_old(2) + delta_T
c        if (T_old(2).gt.Tf) then
c          T_old(2) = Tf
c          melt_flag(2) = 1
c        else
c          melt_flag(2) = 0
c        endif
c put rain precip in bottom layer 
        swe_lyr(1) = swe_lyr(1) + rain
        ro_layer(1) = (swe_lyr(1)/dy_snow(1))*1000
c temperature diff for bottom layer (rain and melt in)
        delta_T = ((swemelt+rain)*xLf)/(Cp_snow*dy_snow(1))
        T_old(1) = T_old(1)+delta_T
        Tdiff = T_old(1)-Tf
c if cold capacity achieved
        if (Tdiff.gt.0) then
          melt_flag(1) = 1
          T_old(1) = Tf
c remaining water goes to runoff          
          runoff = (Tdiff*Cp_snow*dy_snow(1))/xLf
        else
          melt_flag(1) = 0
        endif
c reconfigure snow based on runoff
        if (runoff.gt.0.) then
          swe_lyr(1) = swe_lyr(1)-runoff
          dy_snow(1) = (swe_lyr(1)/ro_layer(1))*1000
        endif
      endif
...
c Account for the accumulation of snow precipitation on the snowpack.
     ...
     &  swe_lyr,ro_nsnow,T_old,Tsfc,tsls_threshold,dt,
     &  melt_flag,soft_snow_d,ro_snow,sum_sprec,sprec_grnd_ml,
     &  sum_prec,prec,sum_runoff,runoff,snow_d,snow_depth,
     &  swe_depth,ml_ret)
       

      print *,dy_snow(1),ro_layer(1)
c Merge layers if the number of layers exceeds some maximum number of
c   layers or if a layer gets thinner than some minimum thickness.
      CALL MERGE_LAYERS_ML_2LAY(JJ,ro_layer,dy_snow,swe_lyr,T_old,
     &  ro_water,max_layers,change_layer,dz_snow_min,melt_flag,Tf)

      print *,dy_snow(1),ro_layer(1)
c Calculate the temperature of each snow layer.
      CALL SNOWTEMP_ML_2LAY(gamma,T_old,Tsfc,JJ,dt,ro_layer,
     &  Cp_snow,Tf,dy_snow,melt_flag)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      SUBROUTINE MERGE_LAYERS_ML_2LAY(JJ,ro_layer,dy_snow,swe_lyr,T_old,
     &  ro_water,max_layers,change_layer,dz_snow_min,melt_flag,Tf)
      ...
      real dy_snow_small,ro_water,dz_snow_min,change_layer
      real percCont,sweDiff,lyRat,Tf
...
c if snow increased the simulated layers to 3      
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      if (JJ.eq.3) then
c calculate SWE to put in main pack
c        sweDiff = (swe_lyr(3)+swe_lyr(2))-0.10
        sweDiff = (swe_lyr(3)+swe_lyr(2))-maxD
c determine percent of lower pack
        percCont = sweDiff/swe_lyr(1)
c swe of lower layer
        swe_lyr(1) = swe_lyr(1) + sweDiff
c set swe to skin layer
        lyRat = swe_lyr(3)/swe_lyr(2)
c        swe_lyr(2) = 0.10
        swe_lyr(2) = maxD
        ro_layer(2) = (lyRat*ro_layer(3))+
     &    ((1-lyRat)*ro_layer(2))
        dy_snow(2) = swe_lyr(2)/(ro_layer(2)/1000)
c properties of lower pack
        ro_layer(1) = (percCont*ro_layer(2))+
     &    ((1-percCont)*ro_layer(1))
        dy_snow(1) = swe_lyr(1)/(ro_layer(1)/1000)
c skin layer temperature of newly fallen snow
        T_old(2) = T_old(3)
c temperature of lower pack
        T_old(1) = (percCont*T_old(2))+
     &    ((1-percCont)*T_old(1))
c reset layers
        JJ = 2
c reset melt flags
        do j = 1,JJ
          if (T_old(j).ge.Tf) then 
            T_old(j) = Tf
            melt_flag(j) = 1
          else
            melt_flag(j) = 0
...
c if simulated snow has two layers
ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      if (JJ.eq.2) then
ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c if the top layer is above maximum SWE
ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c        if (swe_lyr(2).gt.0.10) then
        if (swe_lyr(2).gt.maxD) then
c calculate SWE to put into main pack
c          sweDiff = swe_lyr(2)-0.10
          sweDiff = swe_lyr(2)-maxD
c determine percent of lower pack
          percCont = sweDiff/swe_lyr(1)
c swe of lower layer
          swe_lyr(1) = swe_lyr(1) + sweDiff
c skin layer properties
c          swe_lyr(2) = 0.10
          swe_lyr(2) = maxD
          dy_snow(2) = swe_lyr(2)/(ro_layer(2)/1000)
c properties of lower pack
...
c reset melt flags
          ...
          enddo
ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c if the top layer is less than the maximum SWE
c and the bottom layer is greater than maximum SWE
ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c        elseif (swe_lyr(2).lt.0.10.and.swe_lyr(1).gt.0.20) then
        elseif (swe_lyr(2).lt.maxD.and.swe_lyr(1).gt.(maxD*2)) then
c calculate SWE to put in upper layer
c          sweDiff = 0.10-swe_lyr(2)
          sweDiff = maxD-swe_lyr(2)
c determine percent of top layer
          percCont = sweDiff/swe_lyr(2)
c properties of lower pack
          swe_lyr(1) = swe_lyr(1) - sweDiff
          dy_snow(1) = swe_lyr(1)/(ro_layer(1)/1000)
c properties of skin
c          swe_lyr(2) = 0.10
          swe_lyr(2) = maxD
          ro_layer(2) = (percCont*ro_layer(1))+
     &    ((1-percCont)*ro_layer(2))
          dy_snow(2) = swe_lyr(2)/(ro_layer(2)/1000)
c new temp of skin layer
          T_old(2) = (percCont*T_old(1))+
     &      ((1-percCont)*T_old(2))        
c reset melt flags
          do j = 1,JJ
            if (T_old(j).ge.Tf) then
...
c if neither the top or bottom layer is greater than maximum SWE
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c        elseif (swe_lyr(2).lt.0.10.and.swe_lyr(1).lt.0.20) then
        elseif (swe_lyr(2).lt.maxD.and.swe_lyr(1).lt.(maxD*2)) then
c set to one layer
          JJ = 1
c put SWE int top layer in bottom layer
          swe_lyr(1) = swe_lyr(1) + swe_lyr(2)
c determine percent of lower-layer
          percCont = swe_lyr(2)/swe_lyr(1)
c properties of lower pack
          ro_layer(1) = (percCont*ro_layer(2))+
     &      ((1-percCont)*ro_layer(1))
          T_old(1) = (percCont*T_old(2))+
...
c if one layer, but can be split into two layers
cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c      if (JJ.eq.1.and.swe_lyr(1).ge.0.20) then
      if (JJ.eq.1.and.swe_lyr(1).ge.(maxD*2)) then
c reset layers
        JJ = 2
c set swe and density and temp and melt flag  of layers
c        swe_lyr(2) = 0.10
        swe_lyr(2) = maxD
        swe_lyr(1) = swe_lyr(1)-swe_lyr(2)
        ro_layer(2) = ro_layer(1)
        T_old(2) = T_old(1)
        melt_flag(2) = melt_flag(1)
c calculate new snow depths
        dy_snow(2) = swe_lyr(2)/(ro_layer(2)/1000)
        dy_snow(1) = swe_lyr(1)/(ro_layer(1)/1000)
      endif

cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c if any of the above makes the pack layer smaller than
c the surface layer, then group
cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      if (JJ.eq.2.and.swe_lyr(2).gt.swe_lyr(1)) then
c reset layers
        JJ = 1
        lyRat = swe_lyr(2)/swe_lyr(1)
        swe_lyr(1) = swe_lyr(2)+swe_lyr(1)
        ro_layer(1) = (lyRat*ro_layer(2))+
     &    ((1-lyRat)*ro_layer(1))
        T_old(1) = (lyRat*T_old(2))+
     &    ((1-lyRat)*T_old(1))
        dy_snow(1) = swe_lyr(1)/(ro_layer(1)/1000)
        if (T_old(1).gt.Tf) then
          T_old(1) = Tf
          melt_flag(1) = 1
        else
          melt_flag(1) = 0
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      SUBROUTINE SNOWTEMP_ML_2LAY(gamma,T_old,Tsfc,JJ,dt,ro_layer,
     &  Cp_snow,Tf,dy_snow,melt_flag)
